...
class FetchFromCacheMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: WSGIRequest) -> Optional[HttpResponse]:
        ...
        cache_key = get_cache_key(request, prefix, 'GET', cache=self._wagcache)
        ...
        if response is None and request.method == 'HEAD':
            cache_key = get_cache_key(request, prefix, 'HEAD', cache=self._wagcache)
...
class UpdateCacheMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request: WSGIRequest, response: HttpResponse) -> HttpResponse:
        ...
        if timeout:
            ...
            prefix = _get_prefix(request)
            cache_key = learn_cache_key(request, response, timeout, prefix, cache=self._wagcache)
